...
parser.add_argument("--network",type=str,default="rnn2",help='network name on symbols/')
...
valid=wkiter([vzjdata,vjjdata],batch_size=batch_size,begin=0.7,end=args.end*0.3+0.7,rc=rc,onehot=onehot)
valid2=wkiter([vzqdata,vzgdata],batch_size=batch_size,end=args.end*0.5,rc=rc,onehot=onehot)
valid3=wkiter([vqqdata,vggdata],batch_size=batch_size,end=args.end*0.5,rc=rc,onehot=onehot)

history=AddVal([(valid2,"val2"),(valid3,"val3")])
...
model.fit_generator(train.next(),steps_per_epoch=train.totalnum(),validation_data=valid.next(),validation_steps=valid.totalnum(),epochs=epochs,verbose=1,callbacks=[checkpoint,history])

print(history)
f=open(savename+'/history','w')
try:
  one=history.history['val_acc'].index(max(history.history['val_acc']))
  two=history.history['val2_acc'].index(max(history.history['val2_acc']))
  three=history.history['val3_acc'].index(max(history.history['val3_acc']))

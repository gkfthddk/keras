...
for sha in model._feed_inputs:
  ...
  if(len(sha._keras_shape)==3):
    rc+="r"
onehot=0
test2=wkiter([vzqdata,vzgdata],batch_size=batch_size,begin=args.end*0.2,end=args.end*0.6,rc=rc,onehot=onehot,channel=args.channel,order=args.order,eta=args.eta,etabin=args.etabin)
test3=wkiter([vqqdata,vggdata],batch_size=batch_size,begin=args.end*0.2,end=args.end*0.6,rc=rc,onehot=onehot,channel=args.channel,order=args.order,eta=args.eta,etabin=args.etabin)
...
for ii in range(2,4):
  ...
  outname=savename+"/etabig{}v{}t{}".format(args.eta,iii,ii)
  for j in range(1):
    a,c=next(gen)
    ...
    for i in range(len(c)):
      if(c[i][0]==0):
        ...
        q.append(b[i])
    print("###number",args.pt,args.eta,len(b))
  plt.figure(1)

...
for iii in range(1,4):
  ...
  if(iii==1):
    ...
    for sha in model._feed_inputs:
      ...
      if(len(sha._keras_shape)==3):
        rc+="r"
    onehot=0
    test2=wkiter([vzqdata,vzgdata],batch_size=batch_size,begin=args.end*0.3,end=args.end*0.6,rc=rc,onehot=onehot)
    test3=wkiter([vqqdata,vggdata],batch_size=batch_size,begin=args.end*0.3,end=args.end*0.6,rc=rc,onehot=onehot)
  ...
  test3.reset()
  for ii in range(2,4):
    if(ii==2):gen=test2.next()
    if(ii==3):gen=test3.next()
    x=[]
    y=[]
    g=[]
    q=[]

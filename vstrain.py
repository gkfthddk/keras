...
class BinaryClassifierResponse(object):
    ...
    def append(self, name, test):
        ...
        if("vv" == test):event="validation_"
        if("tt" == test):event="training_"
    ...
    def _draw(self):
        ...
        for i in range(len(self._keys)):
            ...
            if("validation" in self._keys[i]):
                hists[i].SetFillColorAlpha(color,0.7)
        ...
        for i in range(len(self._keys)):
            if("training" in self._keys[i]):hists[i].Draw("EP same")
              ...
              hists[i].Draw("E2 1same")
              hists[i].Draw("L 1same")
        h0.Draw("hist same")
...
for pt in pts:
    ...
    if("rnn" in filename):
      a.append("pepzjrnn{}sgd".format(pt),"vv")
      a.append("pepzjrnn{}sgd".format(pt),"tt")
    if("cnn" in filename):
      a.append("pepzjcnn{}model".format(pt),"vv")
      a.append("pepzjcnn{}model".format(pt),"tt")
    a._draw()

...
parser.add_argument("--jet",type=int,default=0,help='0 or z or not')
...
loaded=np.load("gen{}.npz".format(args.pt))
if(args.stride==3):X=loaded["bdtset"][:,:]
elif(args.jet==0):X=loaded["bdtset"][:,:5*(args.stride)]
elif(args.jet==1):X=loaded["bdtset"][:,5*(args.stride-1):10]
...
if(args.pred==1):os.system("python genxpred.py --pt {} --save {} --stride {} --jet {}".format(args.pt,args.save,args.stride,args.jet))
print(X.shape)

...
for name in namelist:#,'uj','gj']:
  ...
  for num_entry in range(event.GetEntries()):
    ...
    for rot in range(16):
      ...
      if("j" in name):
        oneimgyzx_e2_s=np.zeros((zbin,ybin,xbin+1))
        oneimgyzx_e2_c=np.zeros((zbin,ybin,xbin+1))
      ...
      oneimgyzx_e_s=np.zeros((zbin,ybin,xbin+1))
      oneimgyzx_e_c=np.zeros((zbin,ybin,xbin+1))
...
np.savez_compressed("/home/yulee/keras/rot16ug{}img".format(pt),**imgset)
np.savez_compressed("/home/yulee/keras/rot16ug{}vox".format(pt),**voxels)

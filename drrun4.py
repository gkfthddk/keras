...
if(args.rot==1):
  if(args.voxel==1):
    if("qg" in args.memo):
      loaded=np.load("/home/yulee/keras/rot16ug{}vox.npz".format(args.pt))
    else: 
      loaded=np.load("/home/yulee/keras/rot16{}vox.npz".format(args.pt))
  else:
    if("qg" in args.memo):
      loaded=np.load("/home/yulee/keras/rot16ug{}img.npz".format(args.pt))
    else:
      loaded=np.load("/home/yulee/keras/rot16{}img.npz".format(args.pt))
else:
  if(pix==17):
...
if("egp" in args.memo):
  ...
  ga=imgset["ga"][:,:channel]
  pi=imgset["pi"][:,:channel]
  target="egp"
  ellabel=len(el)*[[1.,0.,0.]]
  galabel=len(ga)*[[0.,1.,0.]]
  pilabel=len(pi)*[[0.,0.,1.]]
  eentcut=int(0.7*len(ellabel))
  gentcut=int(0.7*len(galabel))
  pentcut=int(0.7*len(pilabel))
  X,Y=shuffle(np.concatenate([el[:eentcut],ga[:gentcut],pi[:pentcut]]),np.concatenate([ellabel[:eentcut],galabel[:gentcut],pilabel[:pentcut]]))
  testX,testY=shuffle(np.concatenate([el[eentcut:],ga[gentcut:],pi[pentcut:]]),np.concatenate([ellabel[eentcut:],galabel[gentcut:],pilabel[pentcut:]]))
else:
  if("qg" in args.memo):
    if(channel==-1):

...
try:
  ...
  hist=eval(history[1])
vqqdata="Data/qg_pt_{0}_{1}.root".format(args.pt,int(args.pt*1.1))
vggdata="Data/qg_pt_{0}_{1}.root".format(args.pt,int(args.pt*1.1))
...
for sha in model._feed_inputs:
  ...
  if(len(sha._keras_shape)==3):
    rc+="r"
onehot=0
test2=wkiter([vqqdata,vggdata],batch_size=batch_size,begin=0.,end=1.,rc=rc,onehot=onehot,channel=args.channel,order=args.order,eta=args.eta,etabin=args.etabin,pt=args.pt,ptmin=args.ptmin,ptmax=args.ptmax,unscale=args.unscale,normb=args.normb)
entries=test2.totalnum()
print ("test   ",entries)
#epoch=eval(open(savename+"/history").readline())+1
test2.reset()
#if(args.epoch==None):
f=rt.TFile("{}/qgget.root".format(savename),"recreate")
#else:

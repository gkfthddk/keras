...
class wkiter(object):
  def __init__(self,data_path,data_names=['data'],label_names=['softmax_label'],batch_size=100,begin=0.0,end=1.0,rat=0.7,endcut=1,arnum=16,maxx=0.4,maxy=0.4,istrain=0, varbs=0,rc="rc",onehot=0,channel=64,order=1,eta=0.,etabin=2.4,pt=None,ptmin=0.,ptmax=2.):
    ...
    self.qjet=np.load(dataname1)
    self.gjet=np.load(dataname2)
    print(dataname2)
    self.gEntries=len(self.gjet["pt"])
    if(begin>1):
    ...
    self.a=self.gBegin
    self.qEntries=len(self.qjet["pt"])
    if(begin>1):
    ...
    for i in range(self.gEntries):
      ...
      ##label q=1 g=0
      self.a+=1
      if(self.eta>abs(self.gjet["eta"][self.a]) or self.eta+self.etabin<abs(self.gjet["eta"][self.a])):
        continue
      if(self.pt!=None):
        if(self.pt*self.ptmin>self.gjet["pt"][self.a] or self.pt*self.ptmax<self.gjet["pt"][self.a]):
          continue
      gptset.append(self.gjet["pt"][self.a])
      getaset.append(self.gjet["eta"][self.a])
      if("c" in self.rc):

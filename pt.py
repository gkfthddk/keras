...
pt=100
qf=rt.TFile("Data/zj_pt_{0}_{1}.root".format(pt,int(pt*1.1)),'read')
qjet=qf.Get("jetAnalyser")
gf=rt.TFile("Data/jj_pt_{0}_{1}.root".format(pt,int(pt*1.1)),'read')
...
maxlen=0
for i in range(gjet.GetEntries()):
  qjet.GetEntry(i+0)
  gjet.GetEntry(i+0)

...
dlroc = CompareROC()
#nalist=["naasym","nagru","simple","gruasym"]
#nalabel=["CNNa","GRUa","GRUa+CNNa","GRU+CNN"]

nalist=["gru","rnn","nagru","nagru2"]
nalabel=["GRU","GRU*","GRUa","GRUb"]
#nalist=["rnnasym","gruasym","simple","simple2","asym"]
#nalabel=["GRU*+CNN","GRU+CNN","GRUa+CNNa","GRUb+CNNb","CNN"]
#dlroc.get_yj_result("zj100gruasym","","",legend="Zj,jj")
for i in range(len(nalist)):
  dlroc.get_yj_result("qg100"+nalist[i],"","",legend=nalabel[i])

#dlroc.sort_data_list()
dlroc.compare_roc("ROC curve",filename="plots/onlyrnnroc")
...
for i in range(len(nalist)):
  dlout.get_yj_result("qg100"+nalist[i],"","",ls="-",lw=5,legend=nalabel[i])

dlout.compare_out("Output distribution",filename="plots/onlyrnnout")

...
class wkiter(object):
  def __init__(self,data_path,data_names=['data'],label_names=['softmax_label'],batch_size=100,begin=0.0,end=1.0,rat=0.7,endcut=1,arnum=16,maxx=0.4,maxy=0.4,istrain=0, varbs=0,rc="rc",onehot=0,channel=64,order=1,eta=0.,etabin=2.4,pt=None,ptmin=0.,ptmax=2.,unscale=1,normb=10):
    self.eta=eta
    self.pt=pt
    self.ptmin=ptmin
    self.ptmax=ptmax
    ...
    for i in range(self.gEntries):
      ...
      if("c" in self.rc):
        maxchadpt=1.*sum(self.gjet.image_chad_pt_33)/self.normb
        maxnhadpt=1.*sum(self.gjet.image_nhad_pt_33)/self.normb
        maxelecpt=1.*sum(self.gjet.image_electron_pt_33)/self.normb
        maxmuonpt=1.*sum(self.gjet.image_muon_pt_33)/self.normb
        maxphotonpt=1.*sum(self.gjet.image_photon_pt_33)/self.normb
        maxchadmult=1.*sum(self.gjet.image_chad_mult_33)/self.normb
        maxnhadmult=1.*sum(self.gjet.image_nhad_mult_33)/self.normb
        maxelecmult=1.*sum(self.gjet.image_electron_mult_33)/self.normb
        maxmuonmult=1.*sum(self.gjet.image_muon_mult_33)/self.normb
        maxphotonmult=1.*sum(self.gjet.image_photon_mult_33)/self.normb

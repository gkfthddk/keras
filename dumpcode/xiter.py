...
class wkiter(object):
  def __init__(self,data_path,data_names=['data'],label_names=['softmax_label'],batch_size=100,begin=0.0,end=1.0,rat=0.7,endcut=1,arnum=16,maxx=0.4,maxy=0.4,istrain=0, varbs=0,rc="rc",onehot=0,channel=30,order=1,eta=0,etabin=2.4,pt=None,ptmin=0.,ptmax=2.):
    ...
    for i in range(self.gEntries):
      ...
      gjetset.append([self.gjet.chad_mult+self.gjet.electron_mult+self.gjet.muon_mult,self.gjet.nhad_mult+self.gjet.photon_mult,self.gjet.ptd,self.gjet.major_axis,self.gjet.minor_axis])
    ...
    for i in range(self.qEntries):
      ...
      qjetset.append([self.qjet.chad_mult+self.qjet.electron_mult+self.qjet.muon_mult,self.qjet.nhad_mult+self.qjet.photon_mult,self.qjet.ptd,self.qjet.major_axis,self.qjet.minor_axis])

...
imodel.compile(loss='categorical_crossentropy',optimizer=keras.optimizers.SGD(lr=0.001),metrics=['accuracy'])
plot_model(imodel,to_file='imodel.png')

imodel.fit_generator(train.next(),steps_per_epoch=train.totalnum()/10,validation_data=valid.next(),validation_steps=valid.totalnum(),epochs=args.epoch,verbose=1)

...
vqgdata="Data/qg_pt_{0}_{1}.root".format(args.pt,int(args.pt*1.1))
...
for sha in model._feed_inputs:
  ...
  if(len(sha._keras_shape)==3):
    rc+="r"
onehot=0
test2=miter.wkiter([vqgdata,vqgdata],batch_size=batch_size,begin=0.*args.end,end=1.,rc=rc,onehot=onehot,channel=args.channel,order=args.order,eta=args.eta,etabin=args.etabin,pt=args.pt,ptmin=args.ptmin,ptmax=args.ptmax,unscale=args.unscale,normb=args.normb)
test3=aiter.wkiter([vqqdata,vggdata],batch_size=batch_size,begin=0.*args.end,end=0.6*args.end,rc=rc,onehot=onehot,channel=args.channel,order=args.order,eta=args.eta,etabin=args.etabin,pt=args.pt,ptmin=args.ptmin,ptmax=args.ptmax,unscale=args.unscale,normb=args.normb)

...
#nl=['asuzqbdt{}ptetaptcut','asuzqrnn{}pteta21ptcut','asuzqcnn{}ptetaptcut']
#nl2=['asuqqbdt{}ptetaptcut','asuqqrnn{}pteta21ptcut','asuqqcnn{}ptetaptcut']
...
for j in range(2):
  for i in range(len(nl)):
    if("npz" in nl[i]):
      ...
      if("qq" in nl[i] and j==0):continue
    if(j==0):
      plt.plot([105,210,525,1050],np.array(aucs[i][event[j+2]])*100,
          ':',linewidth=3,label=ll[i]+event[j+2][:-2],marker=mak[i],
          alpha=0.7,color=cl[i],markersize=fs*ms[i])
    if(j==1):
      plt.plot([105,210,525,1050],np.array(aucs[i][event[j+2]])*100,
          ':',linewidth=3,label=ll[i]+event[j+2][:-2],marker=mak[i],
...
plt.xticks([105,210,525,1050],["100\n~110","200\n~220","500\n~550","1000\n~1100"],size=fs*0.8)
plt.yticks([4,5,6,7,8,9,10],size=fs)
plt.grid(alpha=0.6)
plt.legend(fontsize=fs*0.88,ncol=2,)
a1,a2,b1,b2=plt.axis()
#plt.axis((a1,a2,5,10))
#plt.show()
plt.savefig("plots/{}bkg.pdf".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
plt.savefig("plots/{}bkg.png".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
...
for j in range(2):
  for i in range(len(nl)):
    ...
    if("qq" in nl[i] and j==0):continue
    if(j==0):
      plt.plot([105,210,525,1050],aucs[i][event[0]],
          ':',linewidth=3,label=ll[i]+"no cut",marker=mak[i],
          fillstyle='none',color=cl[i],markersize=fs*ms[i])
    if(j==1):
      plt.plot([105,210,525,1050],aucs[i+3][event[0]],
          ':',linewidth=3,label=ll[i]+"$\eta$ cut",marker=mak[i],
          alpha=0.7,color=cl[i],markersize=fs*ms[i])
...
#plt.title("cnnetacut")
#plt.show()
plt.savefig("plots/{}etacut.pdf".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
plt.savefig("plots/{}etacut.png".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
...
for j in range(2):
  for i in range(len(nl)):
    ...
    if("qq" in nl[i] and j==0):continue
    if(j==0):
      plt.plot([105,210,525,1050],np.array(aucs[i][event[2]])*100,
          ':',linewidth=3,label=ll[i]+"no cut",marker=mak[i],
          fillstyle='none',color=cl[i],markersize=fs*ms[i])
    if(j==1):
      plt.plot([105,210,525,1050],np.array(aucs[i+3][event[2]])*100,
          ':',linewidth=3,label=ll[i]+"$\eta$ cut",marker=mak[i],
          alpha=0.7,color=cl[i],markersize=fs*ms[i])
...
a1,a2,b1,b2=plt.axis()
#plt.axis((a1,a2,5,10))
plt.savefig("plots/{}etabkg.pdf".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
plt.savefig("plots/{}etabkg.png".format(save),bbox_inches='tight',pad_inches=0.5,dpi=300)
"""
#plt.show()

...
nalist=[]
nalabel=[]
#nalabel=["GRU","GRU2","grucnn","simple","cnn","cnn2"]
nets=["grugru2","simplecnnsimplecnn2","grusimplecnn"]
#nalabel=["100~110","200~220","500~550","1000~1100"]
#nalabel=["GRU","cnn"]
pts=["100","200","500","1000"]
pt=pts[3]
nalist.append("RNN+CNN")
nalist.append("RNN ensemble")
nalist.append("CNN ensemble")
nalist.append("mixed ensemble")
names.append("ten"+pt+"simple")
for net in nets:
  names.append(net+pt)

dlroc = CompareROC()
dlroc.get_yj_result(names[0],"","",legend=nalist[0])
for i in range(1,len(names)):
  dlroc.get_ens_result(names[i],"","",legend=nalist[i])
  print(nalist[i])

#dlroc.sort_data_list()
dlroc.compare_roc("ROC curve-CNN",filename="plots/ten"+pt+"mixroc")
...
dlout.get_yj_result(names[0],"","",ls="-",lw=5,legend=nalist[0])
for i in range(1,len(nalist)):
  dlout.get_ens_result(names[i],"","",ls="-",lw=5,legend=nalist[i])

dlout.compare_out("Output distribution-CNN",filename="plots/ten"+pt+"mixout")

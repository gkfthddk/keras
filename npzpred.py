...
#test2.reset()
#test3.reset()
loaded=np.load('get{}.npz'.format(args.pt))
#if(args.epoch==None):
...
for jj in range(0,2):
  ...
  q=[]
  for j in range(1):
    bp=model.predict(loaded['gjet{}'.format(ii)],verbose=0)[:,0]
    bpt=loaded['gpt{}'.format(ii)]
    beta=loaded['geta{}'.format(ii)]
    ...
    bp=model.predict(loaded['qjet{}'.format(ii)],verbose=0)[:,0]
    bpt=loaded['qpt{}'.format(ii)]
    beta=loaded['qeta{}'.format(ii)]
    for i in range(len(bp)):
      p[0]=bp[i]
      pt[0]=bpt[i]
      eta[0]=beta[i]

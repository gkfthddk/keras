...
for name in namelist:#,'uj','gj']:
  ...
  count=0
  if("j" in name):
    ymin=-0.0221*ybin/2*1 # 1 tower in one bin #*4 for bigger bin size
    ymax=0.0221*ybin/2*1
    zmin=-0.0221*zbin/2*1
    zmax=0.0221*zbin/2*1
    #ymin=-0.5
    #ymax=0.5
    #zmin=-0.5
    #zmax=0.5
  ...
  for num_entry in range(event.GetEntries()):
    ...
    voxel.append([np.array(oneimgyzx_e_s),np.array(oneimgyzx_e_c)])
    if("j" in name):
      ...
      voxel.append([np.array(oneimgyzx_e2_s),np.array(oneimgyzx_e2_c)])
...
np.savez_compressed("/home/yulee/keras/side45{}img".format(pt),imgset=imgset,voxels=voxels)

...
loaded=np.load("/home/yulee/keras/hardgen{}.npz".format(args.pt))
X=loaded["seqset"][:50000]
Y=loaded["labelset"][:50000]
...
if(args.stride==2):
  if(args.mod==0):
    history=model.fit([X,X],{"output1" : Y,"output2" : Y},batch_size=512,epochs=epochs,verbose=1,validation_split=0.3,callbacks=[checkpoint])
  else:
...
if(args.stride==2):bp=model.predict([X,X],verbose=0)
bpt=loaded["ptset"][50000:]
beta=loaded["etaset"][50000:]
bpid=loaded["pidset"][50000:]
bbdt=loaded["bdtset"][50000:]
chek=[]
if(args.stride==2):
  ...
  for i in range(leng):
    for j in range(args.stride):
      if(label1[i][j]==1):
        ...
        pid[0]=bpid[i]
        ptd[0]=bbdt[i][2]
        cmult[0]=int(bbdt[i][0])
        nmult[0]=int(bbdt[i][1])
        axis1[0]=bbdt[i][3]
        axis2[0]=bbdt[i][4]
      ...
      if(label2[i][j]==1):
        ...
        pid[0]=bpid[i]
        ptd[0]=bbdt[i][7]
        cmult[0]=int(bbdt[i][5])
        nmult[0]=int(bbdt[i][6])
        axis1[0]=bbdt[i][8]
        axis2[0]=bbdt[i][9]
...
if(args.stride==2):
  ...
  bp=model.predict([X,X],verbose=0)
...
full=np.load("/home/yulee/keras/gendr128{}.npz".format(args.pt))
fx=full["seqset"]
fy=full["labelset"]
bp=model.predict([fx[0],fx[1]],verbose=0)
line1="{} roc 12 {} {} mean {} ".format(args.save,round(roc_auc_score(fy[0][:,0],bp[0][:,0]),5),round(roc_auc_score(fy[1][:,0],bp[1][:,0]),5),round(roc_auc_score(np.concatenate([fy[0][:,0],fy[1][:,0]]),np.concatenate([bp[0][:,0],bp[1][:,0]])),5))
print(line1)

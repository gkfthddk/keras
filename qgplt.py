    ...
    def append(self, pt,var):
        ...
        if("chad" in var):
          for k in range(dqjet.GetEntries()):
            ...
            if(dqjet.parton_id==21 or dqjet.parton_id==0):continue
          ...
          for k in range(zqjet.GetEntries()):
            ...
            if(zqjet.parton_id==21 or zqjet.parton_id==0):continue
          ...
          for k in range(dgjet.GetEntries()):
            ...
            if(dgjet.parton_id!=21):continue
          ...
          for k in range(zgjet.GetEntries()):
            ...
            if(zgjet.parton_id!=21):continue
            zghist.Fill(zgjet.chad_mult+zgjet.electron_mult+zgjet.muon_mult)
        else:
          dqjet.Draw(var+">>dqhist","pt<{} && pt >{} && parton_id!=0 && parton_id!=21".format(ptmax,ptmin))
          zqjet.Draw(var+">>zqhist","pt<{} && pt >{} && parton_id!=0 && parton_id!=21".format(ptmax,ptmin))
          dgjet.Draw(var+">>dghist","pt<{} && pt >{} && parton_id==21".format(ptmax,ptmin))
          zgjet.Draw(var+">>zghist","pt<{} && pt >{} && parton_id==21".format(ptmax,ptmin))
        ...
        if(var=='eta'):
          ...
          plt.fill_between(xax,np.append(dg,0),alpha=0.3,linewidth=2,linestyle='--',facecolor="#ffcfdc",edgecolor='C1',label=r"pp$\rightarrow$zg",step='post')
...
for pt in pts:
  for var in varl:
    filename="plots/matchedjet{}{}".format(var,pt)

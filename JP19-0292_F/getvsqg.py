...
class BinaryClassifierResponse(object):
    def __init__(self,
        ...
        self.f = TFile("./save/{}/get.root".format(name2),"read")
        self.f2 = TFile("./save/{}/qgget.root".format(name2),"read")
        print(name2)
        ...
        self._response.append(self.f.Get("dq"))
        self._response.append(self.f.Get("dg"))
        ...
        self._response.append(self.f2.Get("zq"))
        self._response.append(self.f2.Get("zg"))
        ...
        for i in range(len(self._keys)):
            ...
            if("qg" in self._keys[i]):
              if("quark" in self._keys[i]):
                self._response[i].Draw("p>>{}".format(key),"pt<{} && pt >{} && pid!=21 && pid !=0".format(self.ptmax,self.ptmin))
              if("gluon" in self._keys[i]):
                self._response[i].Draw("p>>{}".format(key),"pt<{} && pt >{} && pid==21".format(self.ptmax,self.ptmin))
            else:
        ...
        # Draw
        h0.Draw("hist L")
        for i in reversed(range(len(self._keys))):
...
for pt in pts:
    filename="plots/qgeventsvs{}".format(pt)
